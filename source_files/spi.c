...
static uint8  g_aucTxQueue[3*SPI_BUFFER_SIZE];
                    ...
                    unsigned char p_ucMsgMaxSize)
{
  ...
  {      
        ...
        SET_GPIO_HI( APP_WAKEUP_LINE ); // set wake up signal to the application procesor
        
        do
        {
            if( GET_LO_GPIO_VALUE( KYB4_LINE ) )
            {
                ucResult = 1;
            }
        } while( !ucResult && (RTC_COUNT-ulRtcCount) < USEC_TO_TMR0(10000) ); // wait until app proc confirmation for max 10ms
        
        SET_GPIO_LO( APP_WAKEUP_LINE ); // clean app processor wake up signal
